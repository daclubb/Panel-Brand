...
define( 'NOTIFIER_THEME_NAME', 'Grizzly' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'grizzly-theme' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://grizzly.wegrass.com/notifier.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier() { 
	...
	<div class="wrap">
		...
		<div id="instructions">
		    ...
		    <p>After update Grizzly theme, navigate to <strong>WP-Admin > Grizzly > Setting</strong> and click <strong>Save Options</strong> button. That's all :D</p>
		</div>
...
function is_theme_update() {
	$xml = get_latest_theme_version(NOTIFIER_CACHE_INTERVAL); // Get the latest remote XML file on our server
	$theme_data = get_theme_data(TEMPLATEPATH . '/style.css'); // Read theme current version from the style.css
	return version_compare( $xml->latest, $theme_data['Version'], '>' );
}
